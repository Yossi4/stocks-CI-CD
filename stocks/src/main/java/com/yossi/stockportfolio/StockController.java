...
public class StockController {
   ...
   // GET all stocks, optionally filtered by number of shares
@GetMapping("/stocks")
public ResponseEntity<List<Stock>> getAllStocks(@RequestParam(value = "shares", required = false) Integer shares) {
    try {
        List<Stock> stocks;
        if (shares != null) {
            stocks = stockRepository.findAll().stream()
                    .filter(stock -> stock.getNumberOfShares().equals(shares))
                    .toList();
        } else {
